...
def runge_kutta_log(f, x0, y0, h, n, r = 0.05, K = 500):
    x = x0
    y = y0
    for _ in range(n):
        k1 = h * f(x, y, K, r)
        k2 = h * f(x + h/2, y + k1/2, K, r)
        k3 = h * f(x + h/2, y + k2/2, K, r)
        k4 = h * f(x + h, y + k3, K, r)
        y = y + (k1 + 2*k2 + 2*k3 + k4)/6
        x = x + h
    return y

def runge_kutta_exp(f, x0, y0, h, n, r = 0.05):
    x = x0
    y = y0
    for _ in range(n):
        k1 = h * f(r, y)
...
def euler_log(f, x0, y0, h, n, r = 0.05, K = 500):
    x = x0
    y = y0
    for _ in range(n):
        y = y + h * f(x, y, K, r)
...
def euler_exp(f, x0, y0, h, n, r = 0.05):
    x = x0
    y = y0
    for _ in range(n):
        y = y + h * f(r, y)
...
def grafico_errores_exp():
    ...
    y_rk = np.array([runge_kutta_exp(variacion_poblacional_exponencial, 0, N0, h, i) for i in range(n)])
    y_euler = np.array([euler_exp(variacion_poblacional_exponencial, 0, N0, h, i) for i in range(n)])
...
def grafico_errores_log():
    ...
    y = solucion_logistica(t, N0, 500, r)
    y_rk = np.array([runge_kutta_log(solucion_logistica, 0, N0, h, i) for i in range(n)])
    y_euler = np.array([euler_log(solucion_logistica, 0, N0, h, i) for i in range(n)])
